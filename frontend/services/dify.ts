...
interface ReviewResponse {
  id?: string;
  status: string;
  outputs?: WorkflowOutputs | null;
  error?: string;
}

export async function runSingleReviewWorkflow(
  payload: SingleReviewPayload,
): Promise<WorkflowOutputs | null> {
...
export const resolvePhotoUrl = (photo: unknown): string | undefined => {
  ...
  if (typeof photo === 'object') {
    ...
    if (
      Array.isArray(photoObj.urls) &&
      photoObj.urls.length > 0 &&
      typeof photoObj.urls[0] === 'string'
    ) {
